"""
This file contains an implementation of the small parsimony algorithm.
"""
...
def backtrack_nucs(Tree, node, parent_nuc):
    """
    Backtracks through the tree to determine the sequences of the internal nodes.
    """
    if node.leaf:
        return
    node_nuc = parent_nuc
...
def SmallParsimony(node, index):
    """
    Calculates the small parsimony score for a given node and index.
    """
    if node.leaf:
...
def WholeSmallParsimony(Tree, sequence_length):
    """
    Calculates the small parsimony score for a whole tree.
    """
    score = 0
    relationships = []
...
def main():
    """
    Runs the small parsimony algorithm.
    """
